...
메모리에 값을 저장하려면 먼저 확보하여야 할 메모리 공간의 크기를 결정해야 한다

다시 말해, 몇 바이트의 메모리 공간을 사용해야 낭비와 손실이 없이 값을 저장할 수 있는지 알아야 한다
...
위 예제의 경우 자바스크립트 엔진은 리터럴 100을 숫자 타입의 값으로 해석하고 숫자 타입의 값 100을 저장하기 위해 8바이트(64비트)의 메모리 공간을 확보한다

그리고 100을 2진수로 저장한다
*/

/* 
자바스립트는 숫자 타입의 값을 생성시 "배정밀도 64비트 부동소수점 형식"을 사용한다
*/

/* 
** 데이터 타입에 따라 확보되는 메모리 공간의 크기 **
ES 사양은 문자열과 숫자 타입 외의 데이터 타입의 크기를 명시적으로 규정하고 있지는 않다

따라서 문자열과 숫자 타입을 제외하고 데이터 타입에 따라 확보되는 메모리 공간의 크기는 자바스크립트 엔진 제조사의 구현에 따라 달라질 수 있다

단, ES 사양에 숫자 타입은 "배정밀도 64비트 부동소수점 형식"을 사용한다고 명시되어 있고, 배정밀도 64비트 부동소수점 형식은 8바이트로 숫자를 표현하므로 이 책에서는 숫자 값의 크기를 8바이트로 설명한다
*/
...
이 때 값을 참조하려면 한 번에 읽어 들어야 할 메모리 공간의 크기, 즉 메모리 셀의 개수 (바이트 개수)를 알아야 한다

score 변수의 경우 저장되어 있는 값이 숫자 타입이므로 8바이트 단위로 읽어 들이지 않으면 값이 훼손 된다

그렇다면 컴퓨터는 한 번에 읽어 들여야 할 메모리 셀의 크기를 어떻게 알 수 있나?

score 변수에 숫자 타입의 값이 할당되어 있으므로 자바스크립트 엔진은 scroe 변수를 숫자 타입으로 인식한다
